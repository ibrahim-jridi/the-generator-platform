...
public class MailNotificationDelegate implements JavaDelegate {
    ...
    private static final Logger log = LoggerFactory.getLogger(MailNotificationDelegate.class);

    public MailNotificationDelegate(NotificationService notificationService, UserService userService, GroupService groupService, RoleService roleService) {
    ...
    public void execute(DelegateExecution delegateExecution) throws Exception {
        ...
        if (sendToObj != null) {
            ...
            List<String> idsTo = new ArrayList<>();
            if (users != null) {
                for (UUID user : users) {
                    String email = userService.getEmailById(user).getBody();
                    to.add(email);
                    idsTo.add(user.toString());
                }
            }
            if (groups != null) {
                for (UUID group : groups) {
                    ...
                    for (UUID id : ids) {
                        String email = userService.getEmailById(id).getBody();
                        to.add(email);
                        idsTo.add(id.toString());
                    }
            ...
            sendMailRequest.setTo(to.stream().distinct().collect(Collectors.toList()));
            notificationService.sendMail(sendMailRequest);
            for (String recipient : idsTo.stream().distinct().collect(Collectors.toList())) {

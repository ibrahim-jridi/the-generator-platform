...
public class FileGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all files").get("/services/bsfilemanagement/api/files").headers(headersHttpAuthenticated).check(status().is(200))
            )
                ...
                .repeat(5)
                .on(
                    exec(http("Get created file").get("/services/bsfilemanagement${new_file_url}").headers(headersHttpAuthenticated)).pause(
                        10
                    )
                )
                .exec(http("Delete created file").delete("/services/bsfilemanagement${new_file_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

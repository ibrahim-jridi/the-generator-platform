...
public class FolderGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all folders")
                    .get("/services/bsfilemanagement/api/folders")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new folder")
                        .post("/services/bsfilemanagement/api/folders")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_folder_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created folder").get("/services/bsfilemanagement${new_folder_url}").headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(http("Delete created folder").delete("/services/bsfilemanagement${new_folder_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Folder entity").exec(scn);

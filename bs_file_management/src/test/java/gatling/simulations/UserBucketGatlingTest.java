...
public class UserBucketGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all userBuckets")
                    .get("/services/bsfilemanagement/api/user-buckets")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new userBucket")
                        .post("/services/bsfilemanagement/api/user-buckets")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"idUser\": 0" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_userBucket_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created userBucket")
                            .get("/services/bsfilemanagement${new_userBucket_url}")
                            .headers(headers_http_authenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created userBucket")
                        .delete("/services/bsfilemanagement${new_userBucket_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the UserBucket entity").exec(scn);

...
public class NotificationGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all notifications")
                    .get("/services/bsnotificationmanagement/api/notifications")
                ...
                .exec(
                    http("Create new notification")
                        .post("/services/bsnotificationmanagement/api/notifications")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                ...
                .on(
                    exec(
                        http("Get created notification")
                            .get("/services/bsnotificationmanagement${new_notification_url}")
                            .headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created notification")
                        .delete("/services/bsnotificationmanagement${new_notification_url}")
                        .headers(headersHttpAuthenticated)
                )
                .pause(10)
        );

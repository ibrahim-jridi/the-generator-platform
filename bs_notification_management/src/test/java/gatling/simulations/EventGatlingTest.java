...
public class EventGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all events")
                    .get("/services/bsnotificationmanagement/api/events")
                ...
                .exec(
                    http("Create new event")
                        .post("/services/bsnotificationmanagement/api/events")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"assign\": \"SAMPLE_TEXT\"" +
                                ", \"display\": \"SAMPLE_TEXT\"" +
                                ", \"eventType\": \"SAMPLE_TEXT\"" +
                                ", \"message\": \"SAMPLE_TEXT\"" +
                                ", \"notificationType\": \"SAMPLE_TEXT\"" +
                                ", \"subject\": \"SAMPLE_TEXT\"" +
                                ", \"recipiantId\": null" +
                                "}"
                            )
                ...
                .on(
                    exec(
                        http("Get created event")
                            .get("/services/bsnotificationmanagement${new_event_url}")
                            .headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created event")
                        .delete("/services/bsnotificationmanagement${new_event_url}")
                        .headers(headersHttpAuthenticated)
                )
                .pause(10)
        );

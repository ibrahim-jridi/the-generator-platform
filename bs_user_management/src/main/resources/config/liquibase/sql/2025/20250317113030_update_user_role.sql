...
SET "role_id" = (SELECT id FROM "bs_role" WHERE label = 'BS_PM_INDUSTRIEL' LIMIT 1)
WHERE "user_id" IN (
  ...
  AND NOT EXISTS (
    ...
    WHERE existing.user_id = "bs_rel_user_role".user_id
      AND existing.role_id = (SELECT id FROM "bs_role" WHERE label = 'BS_PM_INDUSTRIEL' LIMIT 1)
    );

-- Insert BS_PM_INDUSTRIEL for users who don’t have it yet
INSERT INTO "bs_rel_user_role" ("user_id", "role_id")
SELECT u.id, r.id
FROM "bs_user" u
         CROSS JOIN (SELECT id FROM "bs_role" WHERE label = 'BS_PM_INDUSTRIEL' LIMIT 1) r
WHERE u.username IN (
                     'pm-ext-1111221', 'pm-ext-1111222', 'pm-ext-1111223', 'pm-ext-1111224',
                     'pm-ext-1111225', 'pm-ext-1111226', 'pm-ext-1111227', 'pm-ext-1111228',
                     'pm-ext-1111229', 'pm-ext-1111230', 'pm-ext-1111231', 'pm-ext-1111232',

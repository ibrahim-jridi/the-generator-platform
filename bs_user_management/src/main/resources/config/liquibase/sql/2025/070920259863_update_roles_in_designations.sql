...
SET role_id = (SELECT id FROM bs_role WHERE label = 'BS_ROLE_PM_RESP_PHARMACOVIG' LIMIT 1)
...
SELECT
    ...
    false
FROM bs_role r
WHERE r.label = 'BS_ROLE_RESP_REG_AFF';
...
WHERE pm_user_id = (SELECT id FROM bs_user WHERE username = 'pm-ext-1111221' LIMIT 1)
 ...
 AND role_id = (SELECT id FROM bs_role WHERE label = 'BS_ROLE_RESP_CTRL_QA' LIMIT 1);

...
/**
 * Performance test for the ConfigurationReport entity.
 */
public class ConfigurationReportGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all configurationReports")
                    .get("/services/bsreportmanagement/api/configuration-reports")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new configurationReport")
                        .post("/services/bsreportmanagement/api/configuration-reports")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_configurationReport_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created configurationReport")
                            .get("/services/bsreportmanagement${new_configurationReport_url}")
                            .headers(headers_http_authenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created configurationReport")
                        .delete("/services/bsreportmanagement${new_configurationReport_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the ConfigurationReport entity").exec(scn);

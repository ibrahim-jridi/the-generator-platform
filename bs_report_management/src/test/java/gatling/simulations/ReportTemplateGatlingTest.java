 ...
 */
public class ReportTemplateGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all reportTemplates")
                    .get("/services/bsreportmanagement/api/report-templates")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new reportTemplate")
                        .post("/services/bsreportmanagement/api/report-templates")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"type\": \"SAMPLE_TEXT\"" + ", \"path\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_reportTemplate_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created reportTemplate")
                            .get("/services/bsreportmanagement${new_reportTemplate_url}")
                            .headers(headers_http_authenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created reportTemplate")
                        .delete("/services/bsreportmanagement${new_reportTemplate_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the ReportTemplate entity").exec(scn);

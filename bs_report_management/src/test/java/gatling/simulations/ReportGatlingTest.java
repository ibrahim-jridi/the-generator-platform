...
public class ReportGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all reports")
                    .get("/services/bsreportmanagement/api/reports")
                ...
                .exec(
                    http("Create new report")
                        .post("/services/bsreportmanagement/api/reports")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                ...
                .on(
                    exec(
                        http("Get created report").get("/services/bsreportmanagement${new_report_url}").headers(headers_http_authenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created report")
                        .delete("/services/bsreportmanagement${new_report_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

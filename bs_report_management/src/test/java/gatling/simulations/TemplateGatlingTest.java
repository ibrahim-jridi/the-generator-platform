 ...
 */
public class TemplateGatlingTest extends Simulation {
        ...
        .on(
            exec(
                http("Get all templates")
                    .get("/services/bsreportmanagement/api/templates")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new template")
                        .post("/services/bsreportmanagement/api/templates")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                ...
                .on(
                    exec(
                        http("Get created template")
                            .get("/services/bsreportmanagement${new_template_url}")
                            .headers(headers_http_authenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created template")
                        .delete("/services/bsreportmanagement${new_template_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Template entity").exec(scn);

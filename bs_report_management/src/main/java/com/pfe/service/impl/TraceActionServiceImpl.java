...
import com.pfe.service.KafkaProducerService;
...
public class TraceActionServiceImpl implements TraceActionService {
    ...
    public TraceActionServiceImpl(ObjectMapper objectMapper, SpringSecurityAuditorAware springSecurityAuditorAware, KafkaProducerService kafkaProducerService) {
    ...
    private void saveTrace(Object savedEntity, Object id, OperationType traceActionEnum,
        ...
        if (shouldTraceAction(savedEntity)) {
            this.kafkaProducerService.sendKafkaEventActivityAudit(traceActionJPABuilder(savedEntity, id, traceActionEnum, details));
            log.debug("traceAction send successfully : {}", traceActionJPABuilder(savedEntity, id, traceActionEnum, details));
        }
    }
    ...
    private String filterOutAuditFields(
        ...
        if (previousState == null) {
            log.warn("Previous entity is null in filterOutAuditFields, skipping processing");
        }

        if (currentState == null) {
            log.warn("Current entity is null in filterOutAuditFields, skipping processing");
        }
    ...
    public UUID getCurrentUserId() {
        return springSecurityAuditorAware.getCurrentAuditor().orElseThrow(

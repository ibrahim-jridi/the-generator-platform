...
@Component({
  selector: 'app-unsubscribe-waiting-list',
  templateUrl: './unsubscribe-waiting-list.component.html',
  styleUrl: './unsubscribe-waiting-list.component.scss'
})

export class UnsubscribeWaitingListComponent implements OnInit{
  private processDefKey: string = 'WAITING_LIST_UNSUBSCRIPTION';
    ...
    this.waitingListService.getWaitingListByUserId(this.userId).subscribe({
      next: (data: any) => {
        ...
        this.camundaService.startUniqueProcessInstanceByKey(this.processDefKey, vars).subscribe(
            ...
            (error) => {
              this.toastrService.onError(this.translatePipe.transform('task.INVALID_ACTIVE_TASK_UNSUBSCRIBE'), this.translatePipe.transform('menu.ERROR'));
            });

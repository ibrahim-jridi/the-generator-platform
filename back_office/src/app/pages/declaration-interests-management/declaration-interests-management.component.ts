...
@Component({
  ...
  templateUrl: './declaration-interests-management.component.html',
  styleUrls: ['./declaration-interests-management.component.scss']
})
export class DeclarationInterestsManagementComponent implements OnInit {
  constructor(
  ...
  ngOnInit() {
    const processKey: string = 'DECLARATION_INTERESTS';
    const profileCompleted = sessionStorage.getItem('profileCompleted');
    const userQualified = !this.tokenUtilisService.isPM() && profileCompleted == 'true';
    if (userQualified) {
      this.startProcessByKeyAndRedirectToValidateTask(processKey);
    } else if (this.tokenUtilisService.isPM()) {
      this.toastrService.onError(this.translatePipe.transform('individual.NOT_ALLOWED_PM'), this.translatePipe.transform('menu.ERROR'));
